...
class WallpaperApp:
    ...
    async def check_single_permission(self, permission_type, permission_name: str) -> bool:
        """Check and request a single permission"""
        try:
            log_error("INFO", "PERMISSION", f"Checking {permission_name}...")
            status = await self.permission_handler.check_permission(permission_type)
            log_error("INFO", "PERMISSION", f"{permission_name} status: {status}")
            
            if status == self.fph.PermissionStatus.GRANTED:
                log_error("INFO", "PERMISSION", f"{permission_name} already granted")
                return True
            
            elif status == self.fph.PermissionStatus.DENIED:
                log_error("INFO", "PERMISSION", f"Requesting {permission_name}...")
                self.show_snackbar(f"📋 Requesting {permission_name}...", ft.Colors.BLUE_700)
                
                result = await self.permission_handler.request_permission(permission_type)
                log_error("INFO", "PERMISSION", f"{permission_name} request result: {result}")
                
                if result == self.fph.PermissionStatus.GRANTED:
                    log_error("INFO", "PERMISSION", f"{permission_name} granted")
                    return True
                else:
                    log_error("WARNING", "PERMISSION", f"{permission_name} denied by user")
                    return False
            
            elif status == self.fph.PermissionStatus.PERMANENTLY_DENIED:
                log_error("WARNING", "PERMISSION", f"{permission_name} permanently denied")
                return False
            
            else:
                log_error("WARNING", "PERMISSION", f"Unknown status for {permission_name}: {status}")
                return False
                
        except Exception as e:
            log_error("ERROR", "PERMISSION", f"Error checking {permission_name}", e)
            return False
    ...
    async def set_wallpaper_async(self, e):
        ...
        try:
            ...
            if not (self.read_permission_granted and self.write_permission_granted):
            ...
            self.show_loading(False)
            self.show_progress(False)
            log_error("ERROR", "WALLPAPER", "Wallpaper operation error", e)
